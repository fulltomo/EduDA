...
export const DA_METHODS = [
  ...
  },
  {
    id: '4DVar',
    label: '4DVar',
    fullName: '4次元変分法 (L-BFGS)',
    fullNameEn: '4D Variational Data Assimilation (L-BFGS)',
    category: 'variational',
    summary: '同化ウィンドウ内の時系列観測を時間一貫性を保ちL-BFGS準ニュートン法で最適化。',
    summaryEn: 'Optimizes initial states across a time window with dynamical consistency using L-BFGS quasi-Newton adjoint gradient optimization.',
    params: [
      {
        key: 'bgErrorVar',
        label: '背景誤差分散 (σb²)',
  ...
  },
  {
    ...
    label: 'PF',
    fullName: '粒子フィルタ',
    fullNameEn: 'Particle Filter (SIR / LPF)',
    category: 'particle',
    summary: '非線形・非ガウス分布を表現可能。標準SIR型と、高次元の次元の呪いを克服する局所型(LPF)を選択可能。',
    summaryEn: 'Represents fully nonlinear and non-Gaussian distributions. Supports standard SIR and Local Particle Filter (LPF).',
    params: [
      {
        key: 'filterType',
        label: 'アルゴリズム方式',
        labelEn: 'Algorithm Type',
        type: 'select',
        options: [
          { value: 'LPF', label: '局所粒子フィルタ (LPF: 推奨)', labelEn: 'Local Particle Filter (LPF)' },
          { value: 'SIR', label: '標準SIR (次元の呪いあり)', labelEn: 'Standard SIR Bootstrap' },
        ],
        default: 'LPF',
      },
      {
      ...
      },
      {
        key: 'localization',
        label: '局所化半径 (L: LPF用)',
        labelEn: 'Localization Radius (LPF)',
        min: 1,
        max: 10,
        step: 1,
        default: 3,
      },
      {
        ...
        label: 'リサンプリング閾値比率 (Neff/M)',
        labelEn: 'Resample Threshold Ratio',
        min: 0.1,
        max: 1.0,
        step: 0.05,
        default: 0.5,
...
export const OBS_MODES = [
  ...
  { id: 'sparse',  label: '疎密観測',   desc: '前半領域（格子点1〜20）のみを集中観測' },
